import 'dart:ui';
import 'package:easy_pay/presentation/themes/colors/app_default_colors.dart';

class AppDefaultColors {
  static AppColors get colors {
    return AppColors(
      primary: const Color(0xFF1A78E5),
      grayBlue: const Color(0xFF637387),
      neutral900: const Color(0xFF212121),
      neutral800: const Color(0xFF303030),
      neutral700: const Color(0xFF3B3E43),
      neutral600: const Color(0xFF656565),
      neutral500: const Color(0xFF8C8C8C),
      neutral400: const Color(0xFF9F9F9F),
      neutral300: const Color(0xFFB3B3B3),
      neutral200: const Color(0xFFC6C6C6),
      neutral100: const Color(0xFFDEDEDE),
      neutral50: const Color(0xFFF7F7F7),
      neutral0: const Color(0xFFFFFFFF),
    );
  }
}
